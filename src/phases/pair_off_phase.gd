...
var name : String = "Pair Off Phase"
...
func start_phase() -> void:
	...
	while new_combat_made:
		...
		for unit in units_in_combat:
			...
			var adjacent_not_in_combat: Array[Variant] = []
			for entity in adjacent:
				if !_unit_in_combat(entity):
	...
	if _selected_unit == null:
		...
		if unit != null and unit.activate_outline._highlight:
			...
			if unpaired_adjacent.is_empty():
				for adjacent_unit in SKTileMap.get_adjacent_units_not_of_team(map_position, team):
					adjacent_unit.can_activate()
	else:
...
func _activate_unpaired() -> void:
	...
	if not any_of_team_remaining:
		for unit in _units_to_pair:
			for adjacent in SKTileMap.get_adjacent_units_not_of_team(unit.get_map_position(), unit.team):
